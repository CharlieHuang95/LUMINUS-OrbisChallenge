...
public class PlayerAI {
    ...
    public void doMove(World world, FriendlyUnit[] friendlyUnits, EnemyUnit[] enemyUnits) {
        ...
        if (counter >= 25) target_nests.clear();
        if (counter >= 25) target_nests2.clear();
        ...
        for (FriendlyUnit unit: friendlyUnits) {
            ...
            if(this_fly.type==1){
                ...
                if(this_fly.steps<=3){
                    ...
                    if (unit.getHealth()>6) path = world.getShortestPath(unit.getPosition(),
                            world.getClosestCapturableTileFrom(unit.getPosition(), null).getPosition(), null
                    );
                    if (path != null) {
